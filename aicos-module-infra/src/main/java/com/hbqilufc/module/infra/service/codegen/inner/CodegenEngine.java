package com.hbqilufc.module.infra.service.codegen.inner;
...
import com.hbqilufc.framework.apilog.core.enums.OperateTypeEnum;
...
import com.hbqilufc.framework.common.util.date.LocalDateTimeUtils;
...
import com.hbqilufc.framework.excel.core.annotations.DictFormat;
import com.hbqilufc.framework.excel.core.convert.DictConvert;
...
import com.hbqilufc.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.hbqilufc.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.hbqilufc.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.hbqilufc.module.infra.enums.codegen.CodegenSceneEnum;
import com.hbqilufc.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.hbqilufc.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.hbqilufc.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 aicos-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 aicos-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.hbqilufc.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/aicos-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/aicos-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "aicos-module-${table.moduleName}/" + // 顶级模块
                "aicos-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "aicos-module-${table.moduleName}/" + // 顶级模块
                "aicos-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "aicos-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "aicos-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "aicos-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
